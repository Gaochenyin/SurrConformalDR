...
SurrConformalDR <- function(df,
  ...
  if(outcome.type == 'Continuous'){
    ...
    df_train_A1 <- data.frame(tau = predict(qrf.Y1.obj,
                                            newdata = df[train.idx, grep('^([XS])',
                                                                         colnames(df),
                                                                         value = TRUE)])$predictions,
                              df[train.idx, grep('^([XA])',
                                                 colnames(df),
                                                 value = TRUE)]) %>%
      filter(A == 1) %>% dplyr::select(-A)

    m.X1q1.obj <- SuperLearner(Y = df_train_A1$tau.1,
    ...
    m.X1q2.obj <- SuperLearner(Y = df_train_A1$tau.2,
                               X = df_train_A1[, grep('([X])',
                                                      colnames(df_train_A1),
                                                      value = TRUE),
                                               drop = FALSE],
    ...
    q.Y1 <- cbind(predict(m.X1q1.obj, newdata = df[, grep('([X])', colnames(df),
                                                          value = TRUE),
                                                   drop = FALSE])$pred,
    ...
    # change for SuperLearner model
    df_train_A0 <- data.frame(tau = predict(qrf.Y0.obj,
                                            newdata = df[train.idx, grep('^([XS])',
                                                                         colnames(df),
                                                                         value = TRUE)])$predictions,
                              df[train.idx, grep('^([XA])',
                                                 colnames(df),
                                                 value = TRUE)]) %>%
      filter(A == 0) %>% dplyr::select(-A)

    m.X0q1.obj <- SuperLearner(Y = df_train_A0$tau.1,
    ...
    m.X0q2.obj <- SuperLearner(Y = df_train_A0$tau.2,
                               X = df_train_A0[, grep('([X])',
                                                      colnames(df_train_A0),
                                                      value = TRUE),
                                               drop = FALSE],
    ...
    q.Y0 <- cbind(predict(m.X0q1.obj, newdata = df[, grep('([X])', colnames(df),
                                                          value = TRUE),
                                                   drop = FALSE])$pred,
  ...
  if(outcome.type == 'Categorical'){
    ...
    # wS
    objY1.XS <- multinom(paste('Y~', paste(grep('^([XS])',
                                                colnames(df),
                                                value = TRUE), collapse = '+')),
                         data = df[train.idx,],
                         subset = A == 1 & D == 1, trace = FALSE)
    probY1.XS <- predict(objY1.XS, newdata = df[, grep('^([XS])',
                                                       colnames(df),
                                                       value = TRUE)],
                         type = 'probs')

    objY0.XS <- multinom(paste('Y~', paste(grep('^([XS])',
                                                colnames(df),
                                                value = TRUE), collapse = '+')),
                         data = df[train.idx,],
                         subset = A == 0 & D == 1, trace = FALSE)
    probY0.XS <- predict(objY0.XS, newdata = df[, grep('^([XS])',
                                                       colnames(df),
                                                       value = TRUE)],
                         type = 'probs')
    ...
    if(ncol(probY1.XS) < length(unique(df$Y))){
      ...
      probY1.XS <- cbind(probY1.XS,
                         matrix(0, nrow = nrow(probY1.XS),
    ...
    if(ncol(probY0.XS) < length(unique(df$Y))){
      ...
      level.NA <- setdiff(unique(df$Y),
                          as.numeric(level.prob))
      probY0.XS <- cbind(probY0.XS,
                         matrix(0, nrow = nrow(probY0.XS),
                                ncol = length(level.NA)))
  ...
  model.obj.wS$mA1.obj <- SuperLearner(Y = as.numeric(df.train2$R.wS <= thetaA1.init)[df.train2$D==1],
  ...
  model.obj.wS$mA1.obj.r <- SuperLearner(Y = as.numeric(df.train2$R.wS <= thetaA1.init)[df.train2$D==1],
  ...
  model.obj.wS$mA0.obj <- SuperLearner(Y = as.numeric(df.train2$R.wS <= thetaA0.init)[df.train2$D==1],
                                       X = df.train2[df.train2$D==1,  grep('^([XSD])',
                                                                           colnames(df.train2),
                                                                           value = TRUE),
                                                     drop = FALSE],
  ...
  model.obj.wS$mA0.obj.r <- SuperLearner(Y = as.numeric(df.train2$R.wS <= thetaA0.init)[df.train2$D==1],
                                         X = df.train2[df.train2$D==1,  grep('^([XD])',
                                                                             colnames(df.train2),
                                                                             value = TRUE),
                                                       drop = FALSE],
  ...
  cutoff.Y1.wS_obs <- tryCatch(uniroot(f = function(theta)eif_theta(theta,
                                                                    df.eval = df.eval,
                                       ...
                                       extendInt = 'yes',
                                       maxiter = 100)$root,
                               error = function(e) quantile(df.eval$R.wS, 1 - alphaCI,
                                                            na.rm = TRUE))

  cutoff.Y0.wS_obs <- tryCatch(uniroot(f = function(theta)eif_theta(theta,
                                                                    df.eval = df.eval,
                                                                    model.obj = model.obj.wS,
                                                                    target.A = 0, wS = TRUE,
                                                                    alphaCI = alphaCI,
  ...
  if(outcome.type == 'Continuous'){
    ...
    cutoff.Y1.wS <- tryCatch(uniroot(f = function(theta)eif_theta(theta,
                                                                  df.eval = df.eval,
                                                                  model.obj = model.obj.wS,
                                                                  target.A = 1, wS = TRUE,
                                                                  alphaCI = alphaCI),
    ...
    cutoff.Y0.wS <- tryCatch(uniroot(f = function(theta)eif_theta(theta,
                                                                  df.eval = df.eval,
                                                                  model.obj = model.obj.wS,
                                                                  target.A = 0, wS = TRUE,
                                                                  alphaCI = alphaCI),
   ...
   # wS
    ...
    if(nested){
      ...
      CI.D0.wS <- conformalIntSplitD0(cbind(df.eval,
                                            lower = lower.tau.wS,
