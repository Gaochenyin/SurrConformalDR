...
initialize.theta <- function(df.train1,
  ...
  ## A == 1
  if(target.A == 1){
    theta.init <- tryCatch(uniroot(function(theta){
      psi <- with(df.train1,
                  - D * (1 - A) *   (1 - alphaCI)
      )
    ...
    extendInt = 'yes',
    maxiter = 100)$root,
    error = function(e)quantile(df.train1$R.wS[df.train1$D == 1],
                                1 - alphaCI,
                                na.rm = TRUE))
  }
  if(target.A == 0){
...
conformalIntSplitD0 <- function(df.D1,
  ...
  if(wS){
    ...
    lowerCI.obj <- SuperLearner(Y = df.D1[trainCI.idx, ]$lower,
                                X = df.D1[trainCI.idx, grep('^([XSA])',
                                                            colnames(df.D1),
                                                            value = TRUE)],
                                SL.library = SL.library)

    upperCI.obj <- SuperLearner(Y = df.D1[trainCI.idx, ]$upper,
                                X = df.D1[trainCI.idx, grep('^([XSA])',
                                                            colnames(df.D1),
                                                            value = TRUE)],
                                SL.library = SL.library)
  ...
  piD.obj <- SuperLearner(Y = df.piD$D,
                          X = df.piD[, grep('([XA])',
                                            colnames(df.piD), value = TRUE)],
                          SL.library = SL.library,
                          family = binomial())

  # compute non-conformity score
  R.CI <- c(pmax(predict(lowerCI.obj, newdata = df.D1)$pred - df.D1$lower,
                 df.D1$upper - predict(upperCI.obj, newdata = df.D1)$pred))
  # fit model on R.CI with trainCI.idx
  mC.wS.obj <- NULL
  if(wS){

    mC.wS.obj <- SuperLearner(Y = R.CI[trainCI.idx],
                              X = df.D1[trainCI.idx,
                                        grep('^[XSA]',
                                             colnames(df.D1), value = TRUE)],
                              SL.library = SL.library)

  }

  mC.woS.obj <- SuperLearner(Y = R.CI[trainCI.idx],
                             X = df.D1[trainCI.idx,
                                       grep('^[XA]',
                                            colnames(df.D1), value = TRUE)],
                             SL.library = SL.library)
  ...
  predict.conformalIntD0 <- function(lowerCI.obj, # model fit for lower bound
    ...
    if(wS){
      ...
      cutoff <- tryCatch(uniroot(f = function(theta_C)eif_thetaC(theta_C,
                                                                 alphaCI = alphaCI.Int),
                                 interval = quantile(R.CI, c(0.01, 0.99),
                                                     na.rm = TRUE),
                                 extendInt = 'yes',
                                 maxiter = 100)$root,
                         error = function(e) quantile(R.CI, .95,
                                                      na.rm = TRUE))
    }else{
    ...
    data.frame(lower = lower.tauD0,
               upper = upper.tauD0)

  }

  CI.D0 <- predict.conformalIntD0(lowerCI.obj = lowerCI.obj,
